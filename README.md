...
sudo helm upgrade argocd-infra  argo/argo-cd --set notifications.enabled=false --set dex.enabled=false --set redis.enabled=true --set server.replicas=0 --set configs.cm.admin.enabled=false --set configs.repositories.gitrepo.url=https://github.com/ThomasInfras/k8s-automated-kind.git --set applicationSet.replicas=0  --namespace argocd-infra --create-namespace
